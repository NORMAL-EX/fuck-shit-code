...
impl ErrorHandlingMetric {
    fn analyze_function_error_handling(&self, func: &Function, language: LanguageType) -> ErrorHandlingScore {
        // 根据不同语言检测错误处理模式
        match language {
            LanguageType::Rust => self.analyze_rust_error_handling(func),
            LanguageType::Go => self.analyze_go_error_handling(func),
            LanguageType::JavaScript | LanguageType::TypeScript => self.analyze_js_error_handling(func),
    ...
    fn analyze_js_error_handling(&self, func: &Function) -> ErrorHandlingScore {
        ...
        let has_async = func.name.contains("async") || func.name.contains("fetch") || 
    ...
    fn analyze_python_error_handling(&self, func: &Function) -> ErrorHandlingScore {
        ...
        let has_io_operations = func.name.contains("read") || func.name.contains("write") ||
                               func.name.contains("open") || func.name.contains("request");
    ...
    fn analyze_c_error_handling(&self, func: &Function) -> ErrorHandlingScore {
        ...
        let has_file_ops = func.name.contains("open") || func.name.contains("read") || 
