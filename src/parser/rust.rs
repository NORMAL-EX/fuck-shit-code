...
impl RustParser {
    ...
    pub fn new() -> Self {
        let function_regex = Regex::new(
            r"^\s*(pub\s+)?(async\s+)?(unsafe\s+)?(extern\s+.*\s+)?fn\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:<[^>]*>)?\s*\(([^)]*)\)"
        ).unwrap();

        RustParser { function_regex }
    }
    ...
    fn count_comment_lines(&self, lines: &[&str]) -> usize {
        ...
        for line in lines {
            ...
            // 检查块注释开始
            if self.starts_block_comment(trimmed) {
                count += 1;
                in_block_comment = !self.ends_block_comment(trimmed);
            }
        }
    ...
    /// # Returns
    /// * `bool` - 是否开始
    fn starts_block_comment(&self, line: &str) -> bool {
        line.starts_with("/*")
    }
    ...
    /// # Returns
    /// * `bool` - 是否结束
    fn ends_block_comment(&self, line: &str) -> bool {
        line.contains("*/")
    }
    ...
    fn try_parse_function(&self, lines: &[&str], start: usize) -> Option<Function> {
        ...
        // 匹配函数声明
        let captures = self.function_regex.captures(line)?;

        // 提取函数信息
        let name = captures.get(5)?.as_str().to_string();
        let params_str = captures.get(6)?.as_str();
    ...
    fn calculate_complexity(&self, function_lines: &[&str]) -> usize {
        ...
        for line in function_lines {
            ...
            complexity += self.count_error_handling(line);
    ...
    fn count_control_flow_keywords(&self, line: &str) -> usize {
        let keywords = [" if ", " else ", " for ", " while ", " loop ", " match "];

        keywords.iter().map(|kw| line.matches(kw).count()).sum()
    }
    ...
    /// # Returns
    /// * `usize` - 错误处理数量
    fn count_error_handling(&self, line: &str) -> usize {
        let mut count = 0;
    ...
    /// # Returns
    /// * `usize` - 分支数量
    fn count_match_arms(&self, line: &str) -> usize {
